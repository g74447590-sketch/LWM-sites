...
export function createTemplateSite(input: { templateId: SiteTemplateId; businessName: string; whatsapp?: string; locale: Locale }): GeneratedSite {
  ...
  return { language: input.locale, businessName: input.businessName.trim(), tagline: copy.tagline, primaryColor: template.colors.primary, accentColor: template.colors.accent, heroTitle: copy.title, heroBody: copy.body, ctaLabel, ctaHref: contactHref(input.whatsapp), templateId: input.templateId, sections: copy.sections.map(([title, body, items], index) => ({ id: `${input.templateId}-${index + 1}`, title, body, items: items.map(([itemTitle, description, price]) => ({ title: itemTitle, description, ...(price ? { price } : {}) })) })) };
}
